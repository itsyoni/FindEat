...
export function DishEditorModal({ dish, onClose, onSaved }: DishEditorModalProps) {
  ...
  return <div className="dish-editor-backdrop [position:fixed] [z-index:100] [inset:0] [display:grid] [place-items:center] [padding:24px] [background:#17171775] [backdrop-filter:blur(5px)] max-[650px]:[padding:0]" role="presentation" onMouseDown={(event) => {
    ...
    <section className="dish-editor [width:min(820px,100%)] [max-height:calc(100vh_-_48px)] [overflow:hidden] [border:1px_solid_#ffffff30] [border-radius:24px] [background:var(--surface)] [box-shadow:0_30px_100px_#0005] [&>form]:[max-height:calc(100vh_-_130px)] [&>form]:[overflow-y:auto] max-[650px]:[width:100%] max-[650px]:[max-height:100vh] max-[650px]:[border-radius:0] max-[650px]:[&>form]:[max-height:calc(100vh_-_82px)] max-[600px]:[height:100dvh] max-[600px]:[max-height:100dvh]" role="dialog" aria-modal="true" aria-labelledby="dish-editor-title">
