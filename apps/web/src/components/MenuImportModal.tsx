  ...
  return (
    ...
    >
      ...
      >
        ...
        {preview && <footer className="flex shrink-0 items-center justify-between gap-3 border-t border-line bg-surface px-6 py-4 pb-[max(16px,env(safe-area-inset-bottom))] max-[640px]:px-4"><p className="m-0 text-xs text-muted max-[560px]:hidden">This adds new sections to <strong className="text-ink">{collection.name}</strong>.</p><div className="ml-auto flex items-center gap-2 max-[480px]:w-full"><button type="button" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl [border:1px_solid_var(--line)] [background:var(--surface)] px-4 text-xs font-extrabold [color:var(--ink)] shadow-sm transition-[background-color,border-color,transform] hover:[background:var(--soft)] active:translate-y-px disabled:cursor-not-allowed disabled:opacity-55 max-[480px]:flex-1" onClick={onClose} disabled={saving}><XIcon size={16} weight="bold" /> Cancel</button><button type="button" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl [border:1px_solid_color-mix(in_srgb,var(--accent)_80%,transparent)] [background:var(--accent)] px-5 text-xs font-extrabold [color:#faf9f6] [box-shadow:0_9px_22px_color-mix(in_srgb,var(--accent)_28%,transparent)] transition-[background-color,box-shadow,transform] hover:[background:color-mix(in_srgb,var(--accent)_88%,#9c2e19)] hover:[box-shadow:0_12px_26px_color-mix(in_srgb,var(--accent)_34%,transparent)] active:translate-y-px disabled:cursor-not-allowed disabled:opacity-55 max-[480px]:flex-1" onClick={() => void confirmImport()} disabled={saving || dishCount === 0}>{saving ? "Importing…" : <><FileArrowUpIcon size={17} weight="bold" /> Import {dishCount} dish{dishCount === 1 ? "" : "es"}</>}</button></div></footer>}
      </section>
    </div>
  );
}
